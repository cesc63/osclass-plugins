<?php
    switch(Params::getParam('paction')) {
        case 'download':
            // only can access navigating via market.osclass.org, is the button
            // download {version} located at item detail.
            ...
            if( $code != '' ) {
                ...
                if( !empty($file) ) {
                    if($file['s_source_file']!='') {
                        error_log( '-------  '. $file['s_source_file'] );
                        // create download url
                        $s_download = market_file_download_url($slug, $file['s_version']);
//                        error_log("$slug   - " . $file['s_version'] .' => '. $s_download );
